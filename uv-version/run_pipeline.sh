    ...
    if command -v uv >/dev/null 2>&1; then
        ...
        for package in "${required_packages[@]}"; do
            if ! python3 -c "import ${package}" 2>/dev/null; then
                log_message "ERROR" "Missing Python package: ${package}"
                log_message "INFO" "Install with: pip3 install ${package}"
                exit 1
            fi
        done
    fi
...
download_data() {
    ...
    if [[ ${#symbols[@]} -gt 0 ]]; then
        ...
        if command -v uv >/dev/null 2>&1; then
            uv run python "${download_script}" --symbols "${symbols[@]}" --config "${CONFIG_FILE}"
        else
            python3 "${download_script}" --symbols "${symbols[@]}" --config "${CONFIG_FILE}"
        fi
    else
        log_message "INFO" "Downloading data for all configured symbols"
        if command -v uv >/dev/null 2>&1; then
            uv run python "${download_script}" --config "${CONFIG_FILE}"
        else
            python3 "${download_script}" --config "${CONFIG_FILE}"
        fi
    fi
...
process_data() {
    ...
    if command -v uv >/dev/null 2>&1; then
        uv run python "${processor_script}"
    else
...
generate_report() {
    ...
    if [[ ${#symbols[@]} -gt 0 ]]; then
        ...
        if command -v uv >/dev/null 2>&1; then
            uv run python "${report_script}" --symbols "${symbols[@]}" --config "${CONFIG_FILE}"
        else
            python3 "${report_script}" --symbols "${symbols[@]}" --config "${CONFIG_FILE}"
        fi
    else
        log_message "INFO" "Generating report for all symbols"
        if command -v uv >/dev/null 2>&1; then
            uv run python "${report_script}" --config "${CONFIG_FILE}"
        else
            python3 "${report_script}" --config "${CONFIG_FILE}"
        fi
    fi
